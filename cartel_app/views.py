...
def obtener_datos_gym():
     if settings.DEBUG:
         url = 'http://127.0.0.1:8000/api/socios/'  # URL local para desarrollo
     else:
         url = 'http://159.223.115.234:8000/api/socios/'  # URL remota
     
     try:
         logging.debug(f"Haciendo petición GET a: {url}")
...
def actualizar_clases_socio(socio_id, nuevas_clases):
     ...
     if settings.DEBUG:
         url = f'http://127.0.0.1:8000/api/socios/{socio_id}/'
     else:
         url = f'http://159.223.115.234:8000/api/socios/{socio_id}/'
     
     try:
         data = {'clases_restantes': nuevas_clases}
         logging.debug(f"Haciendo petición PATCH a: {url} con datos: {data}")
         response = requests.patch(url, json=data)
         response.raise_for_status()
         logging.debug(f"Respuesta de la API (actualizar clases): {response.status_code}, {response.text}")
         return True
     except requests.exceptions.RequestException as e:
...
def registrar_ingreso_gym(dni_socio, clases_restantes, nombre_socio, apellido_socio):
     ...
     if settings.DEBUG:
         url = f'http://127.0.0.1:8000/api/registrar-ingreso/'
     else:
         url = f'http://159.223.115.234:8000/api/registrar-ingreso/'
     ...
     try:
         ...
         logging.debug(f"Haciendo petición POST a: {url} con datos: {data}")
         response = requests.post(url, json=data)
         response.raise_for_status()
         logging.debug(f"Respuesta de la API (registrar ingreso): {response.status_code}, {response.text}")
         return True
     except requests.exceptions.RequestException as e:
...
def bienvenida(request):
     ...
     if request.method == 'POST':
         ...
         if form.is_valid():
             ...
             refrescar_pagina = True  # Activar la recarga aquí, siempre que el form sea valido
             datos_gym = obtener_datos_gym()
 
             if datos_gym:
                 for socio in datos_gym:
                     print(f"Comparando DNI: socio['dni']={socio['dni']}, dni={dni}") # Imprime los valores a comparar
                     if socio['dni'] == dni:
                         socio_encontrado = socio
                         mensaje = f"Bienvenido, {socio_encontrado['nombre']} {socio_encontrado['apellido']}!" # Cambiado aquí
                         ...
                         if socio_encontrado.get('tipo_mensualidad'):
                             if socio_encontrado['tipo_mensualidad'].get('tipo') == "12 clases":
                                 if socio_encontrado['clases_restantes'] > 0:
                                     ...
                                     # Descontar una clase
                                     nuevas_clases = socio_encontrado['clases_restantes'] - 1
                                     # Actualizar clases en la API
                                     if actualizar_clases_socio(socio_encontrado['id'], nuevas_clases):
                                         mensaje_clases = f" Te quedan {nuevas_clases} clases."
                                         socio_encontrado['clases_restantes'] = nuevas_clases # Actualizamos el valor para la vista
                                         print("Clases actualizadas correctamente, y variable refrescar_pagina seteada a True")
                                     else:
                         ...
                         datos_socio_mostrar = {
                             ...
                             'tipo_mensualidad': socio_encontrado.get('tipo_mensualidad', {}).get('tipo', 'Sin mensualidad'),
                         ...
                         if registrar_ingreso_gym(dni, socio_encontrado['clases_restantes'], socio_encontrado['nombre'], socio_encontrado['apellido']):  # Pasar el DNI del socio y clases restantes
            ...
            refrescar_pagina = False # Para que no se recargue si el form no es valido
